...
export class ConversationInstance {
  ...
  async startConversation(config: any) {
    ...
    this.state.timeLimit = config.time_limit || 60; // Default 60 minutes
    this.state.messageCount = config.message_count || 1; // Default 1 message
  ...
  async typeAndSendMessage(text: string): Promise<boolean> {
    ...
    try {
      ...
      // For auto mode, send the message automatically
  ...
  async typeIntoContentEditable(element: HTMLElement, text: string): Promise<void> {
    return new Promise(async (resolve) => {
      try {
        ...
        // Character-by-character typing for content editable
